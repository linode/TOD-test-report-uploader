...
def merge_xml_files(input_dir):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith('.xml'):
            ...
            total_failures += int(testsuites_element.get('failures')) if testsuites_element.get(
                'failures') is not None else 0
            total_errors += int(testsuites_element.get('errors')) if testsuites_element.get('errors') is not None else 0
            total_skipped += int(testsuites_element.get('skipped')) if testsuites_element.get(
                'skipped') is not None else 0
    ...
    try:
        ...
        log_and_print(f"{timestamp}:XML content successfully over-written to " + output_xml_file)
